...
# ── 基线（2026-09-25 由 examples/EX2022A 实跑得到）──────────────────────────
# ⚠ 口径：2026-09-25 起默认 `ref_preset="horstwood2016"`（Horstwood et al. 2016
#   表 S2 的 CA-ID-TIMS 推荐值）。要复现 2026-09-25 之前的旧数字，跑
#   `--ref-preset repo`；两套口径的差异是**整体平移**，行数/域结构/MSWD 都不变。
ROWS = {"结果": 83, "标样QC": 2, "深度剖面域": 91, "剖面窗口": 1677,
        "不分域年龄": 48}
ROLES = {"样品": 48, "主标": 21, "监控标样": 14}
QC = {                       # 标样 -> (加权平均年龄 Ma, 偏差 %)
    "91500": (1060.2176, -0.3096),
    "Ple": (343.3966, 1.8497),
}
AGE = {"中位": 458.1074, "5%": 420.2161, "95%": 643.5834}   # 只统计样品
CONC = {"中位": 101.2916, "90-110%占比": 0.9375}
...
DOC_FIG = {
    "S43": {"整段": 458.7, "窗口数": 35, "未归域": 0, "整段MSWD": 1.11, "上限": 1.49,
            "积分": 459.3, "域": {}},
    "S34": {"整段": 497.2, "窗口数": 35, "未归域": 0, "整段MSWD": 5.08, "上限": 1.49,
            "积分": None,
            "域": {"D1": (469.6, 13, 1.40), "D2": (515.2, 22, 0.88)}},
    "S18": {"整段": 458.8, "窗口数": 35, "未归域": 8, "整段MSWD": 3.39, "上限": 1.49,
            "积分": None,
            "域": {"D1": (471.8, 7, 0.15), "D2": (442.5, 12, 0.86),
                   "D3": (474.9, 8, 1.96)}},
}
