...
| 不会用命令行的同事 | **重新打包的 exe** ＋ `使用说明.txt` | 双击即用，但**必须先重建**（`dist\` 里那个是旧的，见 §7） |
...
- **`dist/` 里那个 exe 不能直接发给别人**（2026-09-26 实测）。它叫
  `ZirconUPbTool.exe`，构建时间 **2026-09-15 05:05** —— 而本仓库的**第一个提交**
  是 2026-09-19 08:48，改名成 DRUID 是 9-19 02:10 的 `9b62417`。
  两条一夹就得出：**它用的是改名前的源码，而那份源码不在任何提交里** ——
  发出去之后，对方算出的数属于哪一版、怎么复现、坏了怎么修，全都无从谈起。
  同理，它**必然**没有 9-15 之后加的任何东西（`--ref-preset` 口径档、`claim_scope`、
  以及后续的检查项与 sheet），所以它算出的年龄与文档站上写的示例数字对不上。
  **要发 exe 就先双击 `打包成exe.bat` 重建**（产出 `dist\DRUID.exe`，约 47 MB；
