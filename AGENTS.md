...
⚠ **上表里 91500 的偏差 −0.25% 含一个 +0.0602% 的常数地板** —— 因为 `constants.py` 里
91500 的 `R68` 与 `age_Ma` 不自洽。第 ⑩ 步二次校正会把这个地板从**样品**年龄里
吸收掉，但**主标 QC 的"偏差%"**与**无监控标样批次的报出值**都还带着它。
细节、量级与决定见 **§10**。
...
差 **+0.0602%**。本仓**已决定按第一版锁死、不改**（理由见本节末）。
由 `python tools/ref_value_impact.py` 可随时把这个差异重算一遍（只读、实跑 5 遍示例批次）。
...
|---|---|---|---|---|
| **仓库现值（＝第一版）** | 0.179280 | 1062.4 | **1063.04** | **+0.0602%** |
| Wiedenbeck et al. (1995) | 0.179170 | 1062.4 | 1062.44 | +0.0036% |
| Horstwood et al. (2016) | 0.179365 | 1063.51 | 1063.50 | −0.0005% |
...
### 10.4　决定与代价（2026-09-25）

**按第一版锁死、不改。** 理由：改哪一头（比值或年龄）都会让全部年龄与已发表数字
不再可比。要付的代价必须记住三条：

1. 主标 QC 的"偏差%"带一个 **+0.0602% 的常数地板**（示例批次实际报出 −0.2490%）；
2. **未校正列**会随口径平移 —— 报未校正数时必须说明用的是哪套参考值；
3. **无监控标样的批次**（如桂北：样品仓放不下 Ple）拿不到二次校正的吸收，
   这份平移会**原封留在报出值里**（0.1–1.2 Ma 量级）→ 这类批次**必须显式写明
   用的是哪一套 91500 参考值**。qc 层已对此给两条 WARN
   （`calibration.secondary_spots`、`uncertainty.sigma_ext_source`，后者会把
   σext 的 0.70% / 0.25% 标成"写死的假设值"）。

改 `constants.py` 会让 `qc.py` 的 `reference.self_consistency` 检查变绿 ——
**不要为了让它变绿而改**。那条检查现在**故意报 WARN**，它就是用来钉住"这里有一个
已知的上游口径不自洽"；基线由 `tests/test_qc.py::test_reference_self_consistency_flags_the_91500_anchor`
守着（断言 `R68==0.17928`、`age_Ma==1062.4`、`0.058 < diff_pct < 0.062`）。
