...
| 页面 | 什么时候看 |
|---|---|
| `docs/index.html` | 想知道"这工具到底解决什么问题"时。落地页：一句话价值主张、三条设计原则、**先看这三个数**，以及一张从真实示例批次算出的「分域前后」三联图（均一 / 核边分明 / 复杂变化三种剖面形态） |
...
- 仓库布局：`druid/` 代码 · `examples/EX2022A/` 示例批次（85 个测点，样品名
  匿名成 `S01`…`S48`，标样保留）· `tests/` 自检 · `docs/` 文档站（四份页面）· `tools/` 文档图生成脚本
