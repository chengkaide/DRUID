...
| 页面 | 什么时候看 |
|---|---|
| `docs/index.html` | 想知道"这工具到底解决什么问题"时。落地页：一句话价值主张、三条设计原则、**先看这三个数**，以及一张从真实示例批次算出的「分域前后对比图」 |
