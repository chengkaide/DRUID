...
按 `<!-- ==== 首屏预览：……==== -->` 与 `<!-- ==== 首屏预览结束 ==== -->` 两个
标记就地注入，出三样东西 —— 一个真实测点的逐深度剖面（默认 `S10`）、三条质控判据的
...
⚠ **首屏那个测点是挑出来的，判据是"一眼看懂"，不是"落差最大"**：三条 ——
① 全部窗口都归进年龄域（图上没有一片空心点要解释）、② 每个域的 MSWD 都在限内
（拆出来的每段本身都是常数）、③ 整段 MSWD 又明显超限（不分域就是错的）。
示例批次里同时满足的有 `S04 / S10 / S20`，取 `S10`。（不许用 `S43 / S34 / S18`
—— 那三个在 §02 的三联图里。）**换测点**：`python tools/gen_docs_preview.py --spot S20`。
`S21` 被换掉是因为它只满足第 ③ 条：34 个窗里 20 个没通过相容性检验、25 个是空心点，
首屏看到的第一眼是一片问号。
图注里"两个/三个年龄域……"那句以及"整批 N 个测点"那句都是**算出来的**
（`spot_label()` / `len(res.overall)`），不是常量 —— 常量换个测点就开始撒谎。
