...
差 **+0.0602%**。用户 2026-09-25 的决定是**按仓库第一版锁死、不改**
（理由：改哪一头都会让全部年龄与已发表数字不再可比）。

本脚本的职责不是"要不要改"，而是回答**"如果换口径，差别到底多大"** ——
把那个 0.0602% 换成样品年龄上看得见的 Ma，并和不确定度摆在一起比。
...
def harvest(res, c: dict) -> dict:
    ...
    #   那一列（它自己的第 200 行同一套判断），基线中位 458.07 Ma 就是它算出来的。
...
def main() -> int:
    ...
    w()
    w(f"基线是**仓库现值**那行。样品数 {base['n_smp']}，主标点数 {base['n_primary']}；"
    ...
    w("3. **改了会断可比性。** 本仓现值与已发表数字同源，动它就是让历史结果与"
      "新结果不可直接比较；所以用户 2026-09-25 拍板**锁死第一版、不改**，"
      "并把这里量出来的量级写进 `AGENTS.md` 备查。")
